...
public class AssignCookies {
    ...
    * */
    ...
    public static int findContentChildren(int[] children, int[] cookies) {
        ...
        quickSort(children, 0, childrenLen - 1);
        quickSort(cookies, 0, cookiesLen - 1);
        int result = 0;
        int j = 0;
        for (int child : children) {
            while (j < cookiesLen && child > cookies[j])
                j++;
            if (j == cookiesLen) break;
            result++;
            j++;
        }
        return result;
    ...
    // Implement a quick sort for this problem.
